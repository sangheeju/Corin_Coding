...
def browse_dest_path():
    ...
    # if folder_selected is None: # askdirectory()를 썼을때 사용자가 취소를 누르면 빈 문자열이 들어오도록 되어있음
    if folder_selected == "": 
        return
...
def merge_image():
    ...
    ########################################
        ...
        image_sizes = [] # [(width1, height1), (width2, height2), ...] 튜플로 저장하고 리스트에 넣는 작업
        if img_width > -1:
            # width 값 변경
            ...
            # width과 height값이 저장, 이것들을 tuple로 묶는데 x= images, 이미지 불러온 것들 내에서 size값을 가져와서 하나씩 넣어줌 
